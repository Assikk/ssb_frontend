<template>
  <div class="main">
    ...
    <div class="main__container px-6 xl:px-0 bg-white/70">
      ...
      @changePage="changePage"/>
      <Navbar
      :list="activePage && activePage.child"
      class="w-full xl:w-4/5"/>
    </div>
      <Nuxt class="pb-[60px] lg:pb-[120px]" />
      <MobileMenu class="block lg:hidden"
      @changePage="changePage"/>
      <Footer class="hidden lg:block"
      @changePage="changePage"/>
      <MobileFooter class="block lg:hidden mb-[65px]"
      @changePage="changePage"/>
      <transition-group name="fade">
        <ApplicationDefault key="application" v-if="isApplication"/>
        <Success key="success" v-if="isSuccess"/>
        <ApplicationVacancy key="applicationVacancy" v-if="isApplicationVacancy"/>
